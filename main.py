...
app = Flask(__name__)
# ACCESS_TOKEN = 'EAAGkB2Vgav0BACdZAqcbwO1ejgm3V5nLC7MRXFZBXBcZCWAs2d5GnDZADsrZB6duIYZBV1ej7zNHyeFz3kTOrX2mhyFcCUO4ZBp8hLncGG9DukWJcMy69NESiCp0zYBNmktFz0tiwyQXcc6gAgA5Js7qxwTyilBu78VA2XZCZC4J4ORYlQ5JDQuh2V8VLIQQ1nCCd6iLJBNxvWwZDZD'
                ...
                if message.get('message'):
                    ...
                    if message['message'].get('text'):
                        ...
                        if not message['message'].get('is_echo'):
                            ...
                            conn.commit()
                            conn.close()
                            all_conversation(response_sent_text, firstText, patList, docList, recipient_id)
    ...
    if countt == 0:
        if recipient_id not in patientList:
            if "thanks" in firstText or "thank you" in firstText:
                send_message(recipient_id, "you are welcome Dr " + " <3 ")
            ...
            if "thanks" in firstText or "thank you" in firstText:
                ...
                send_message(recipient_id, "ok " + " <3 ")

            elif "good bye" in firstText or "Good Bye" in firstText or "Good bye" in firstText or "bye" in firstText or "Bye" in firstText:
                send_message(recipient_id, firstText + " " + " <3 i wish you getting better soon ^_^ ")

            elif similar(firstText,
                         "i have swollen in breasts i have lumps in my breasts i have tumors in my breasts i have tumefy in my breasts i have tumor in my breasts i have neoplasm in my breasts i have growth in my breasts i have tumefaction in my breasts the breast may be painful when touched but does not suffer pain if do not touch it") < 0.01 and firstText not in words and (
                    "patient" not in firstText or "Patient" not in firstText or "PATIENT" not in firstText or "doctor" not in firstText or "Doctor" not in firstText or "DOCTOR" not in firstText or "DR" not in firstText or "dr" not in firstText or "Dr" not in firstText):
                send_message(recipient_id,
                             "Sorry I can not understand your words :( Please describe what you feel ^_^ ")

            elif similar(firstText, "What is the information about?") >= 0.6 or similar(firstText,
                                                                                        "I want/need to know information about") >= 0.6 or similar(
                ...
                for item in allDiseaseNames:
                    if item[0] in firstText or item[0] in firstText.title() or item[0] in firstText.upper() or item[
                        0] in firstText.capitalize():
                        count = count + 1
                ...
                if len(finalSymptomsList) > 0:
                    ...
                    if size1 == size2:
                        ...
                        for item in SymptomsIDs:
                            cur2 = ct.execute("SELECT diseases_id FROM diseases_symptoms_relation WHERE symptoms_id= ?",
                                              (item[0]))
                        ...
                        for i in range(len(coun)):
                            if coun[i] == maxCount:
                                cur3 = ct.execute("SELECT name FROM diseases WHERE diseases_id= ?",
                                                  (SuccDiseases[i][0]))
                                cur4 = ct.execute("SELECT specialty FROM diseases WHERE diseases_id= ?",
                                                  (SuccDiseases[i][0]))
                                txt = "We recommend that you may be have " + str(
                                    cur3.fetchall()[0][0]) + " and you need to visit a doctor in " + str(
                                    cur4.fetchall()[0][0]) + " specialty"
                                send_message(recipient_id, txt)
                        ct.commit()
                        ct.close()
                        # Count=0
                        send_message(recipient_id, "Do you want to talk to a doctor ? <3 ")
                        global patID
                        patID = recipient_id
                        countt += 1
                    ...
                    message = "ok " + " <3 please describe more details about what you feel ^_^ "
                    send_message(recipient_id, message)
...
#
# app = Flask(__name__)
# # ACCESS_TOKEN = 'EAAGkB2Vgav0BAGxYf5wDJZCobrfgKRNZAVFnsN0r7mDJtowdZBh4DtFkCT5DEIaEpnFyAWD4W11qbw2Q69k25vVFoXVjLix9V45mZCDo0CKrvqfjyWGO6hb2bjKNk0GAdVyHhYkxcXq6blSY0HzC5rCyjBUkZBgCqPZAzEKjL2Y0KDZAhMc4hhGslCWMSYJYD2ZAKJS8HuiOSAZDZD'
# ACCESS_TOKEN = os.environ["access"]
# # This is API key for facebook messenger
# API = "https://graph.facebook.com/v13.0/me/messages?access_token="+ACCESS_TOKEN
# VERIFY_TOKEN = 'hello#100'
